...
public class TestRailsIntegration {
	...
	public String generateRunName(String appVersion){
		Initializer.propInitNoExtentReportMess();
		GetPropertiesFromSysOrConfig props = new GetPropertiesFromSysOrConfig();

		String platformToUse = props.getPropertyFromSysOrConfig("platform");
		String buildIDToUse = null;
		if(platformToUse.equalsIgnoreCase("IOS")){
			buildIDToUse = props.getPropertyFromSysOrConfig("msAppCenterIOSBuildIDToUse");
		} else if(platformToUse.equalsIgnoreCase("android")){
			buildIDToUse = props.getPropertyFromSysOrConfig("msAppCenterAndroidBuildIDToUse");
		}
		String usingBrowserStack = props.getPropertyFromSysOrConfig("runningBrowserStack");

		return "MGO_NativeTests_" + PathGeneration.getTypeOfRunString() + "_" + "runBStack_" + usingBrowserStack + "__version_" + appVersion + "_" + "msAppCenterID_" + buildIDToUse;
		//runs on test rails are specific to an environment and country, even on multiruns, so are therefore appended to the run name here. Identify runs from the same multirun via ID number
	} //run name regenerated with each new run
